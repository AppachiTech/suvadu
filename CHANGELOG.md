...
- **Visual consistency pass across `suv search`, `suv agent dashboard`, `suv agent prompts`, `suv agent stats`, `suv stats`, `suv skills`, `suv bookmarks`, and `suv aliases suggest`** — all now share the same centered `SUVADU <SCREEN>` title and footer badge styling with Quit listed first; the search-driven screens also share an always-on live-filter search box and `Ctrl+<letter>` shortcuts. The dashboard's agent/risk summary and the prompt detail view's session info are now shown in their own boxed panels instead of dense single lines, agent stats' cards use rounded borders, and its High Risk Commands table now fills the available width instead of truncating commands at a fixed 30 characters.
