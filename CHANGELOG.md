...
- **Characters typed while compact recall was opening are no longer swallowed** — an inline viewport has to ask the terminal where the cursor is, and reading that reply from `/dev/tty` also consumed anything typed in the same moment. Typing straight into `Ctrl+R` therefore lost its first characters and searched for something nobody typed. Those keystrokes are now replayed into the query before the first frame. Replay stops at the first control byte: an escape sequence is not text, and typing its bytes would put `[A` in the query box.
