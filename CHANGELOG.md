...
### Fixed
- The `OpenCode` plugin's prompt cache now redacts secrets and uses the configured `agent.prompt_capture_max_chars` length, via a new `suv hook-opencode-prompt` command, instead of writing the raw prompt straight to disk truncated at a hardcoded 500 characters. Previously this let secrets in a prompt reach the cache (and `entries.context["agent_prompt"]`) unredacted, and any prompt over 500 characters couldn't be linked to its command by `suv agent session`/`suv sessions` since the cached and imported copies no longer matched exactly. The cache also now resolves the project-local config for the session's own directory (the same one session import already used), rather than only the global config, so a `.suvadu.toml` overlay that changes redaction or the length cap is honored consistently by both instead of silently reintroducing the same mismatch (or leaving a secret unredacted when a project turns redaction on). Existing users must rerun `suv init opencode` to install the updated plugin.
- `resolve_current_agent_session` no longer silently returns zero candidates when explicitly passed `agent: "opencode"` — the cwd-based fallback's agent filter previously only recognized Codex and Claude Code and matched every other value (including `opencode`) against a fallthrough that excluded all sessions. `native_id` resolution also now accepts `opencode` (mapping to the `opencode-<id>` session ID), matching the existing Codex/Claude Code support.
