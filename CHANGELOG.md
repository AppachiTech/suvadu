...
- Cross-agent MCP session summaries: `list_agent_sessions`, `get_agent_session`, and the `summarize_agent_session` prompt let any connected agent summarize a captured session by ID. Optional `save_session_summary` requires `mcp.allow_session_summaries = true`, current source revision, and session evidence IDs.
- Unified session browser: `suv sessions` now combines human terminal sessions, command-backed agent sessions, and native Codex sessions in one picker. `Ctrl+T` cycles All/Human/AI; AI timelines show captured prompts, final responses, commands, model changes, and the latest cumulative provider-reported token total, including sessions with no commands.
