...
/// hook). On `session.idle` -- and once more on plugin `dispose()` as a
/// best-effort final flush -- it fetches this session's full message
/// history via `OpenCode`'s own documented API and hands it to `suv
/// hook-opencode-session`, which does the incremental import.
...
async function importSession(client, sessionID, directory) {
  try {
    ...
    spawnSync(
      ...
      { input: JSON.stringify(messages), stdio: ["pipe", "ignore", "ignore"], timeout: 10000 }
    );
  } catch {
...
mod tests {
    ...
    fn opencode_plugin_keeps_bash_forwarding_and_adds_idle_triggered_import() {
        ...
        // The old per-message prompt-caching path is gone -- prompts now come
        // from the idle-triggered import instead.
