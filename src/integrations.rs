...
/// with their turn). On `session.idle` -- and once more on plugin
...
///
/// The default export must be `{ id, setup }` (an object), not a bare
/// `(input) => Hooks` factory function -- the installed `opencode` CLI
/// (v2.0.1) rejects the latter with "Plugin must export a default
/// definition with an id and an effect or setup function" and refuses to
/// load the plugin at all. The published `@opencode-ai/plugin` npm
/// package's types (pinned at 1.18.30, the latest available) still
/// describe the bare-function shape; the CLI's actual runtime contract has
/// moved on without a matching SDK release, confirmed empirically against
/// a real local `opencode` install rather than against those types.
