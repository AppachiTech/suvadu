...
pub fn run_search(
    ...
    // Teardown is best effort, and deliberately ignores its errors: by this
    // point the user may already have accepted a command, and a terminal
    // that stops answering during cleanup must not turn that selection into
    // an error with nothing on the prompt. The guard still restores the
    ...
    // surrounding context intact. `clear` asks for the cursor position, so
    // it only runs on a surface that is genuinely inline — after a fallback
    // the UI is on the alternate screen, which the guard leaves instead.
